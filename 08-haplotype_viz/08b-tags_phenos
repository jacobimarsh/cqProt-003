...
##GETTING alt AF 
echo "SITE,COUNTINFO" >> ACAN_tagSNPs.txt
awk 'FS = "," {print $4}' pdifftags.csv | tail -n +2 | grep -f - sort_protein_Gm20.SNPs.id.biallic_maf_0.01_geno_0.1.vcf | sed -e 's/^20\t//' |sed -e 's/\t20_.*AC/,AC/' | sed -e 's/\tGT.*//' | sed 's/..=//g' >> ACAN_tagSNPs.txt
R
library(tidyverse)
